...
from .transforms import build_transforms, build_transforms_RGB
...
class ImageDataManager(BaseDataManager):
    ...
    def __init__(self,
        ...
        transform_train = build_transforms_RGB(self.height, self.width, is_train=True)
        transform_test = build_transforms_RGB(self.height, self.width, is_train=False)

        print("=> Initializing TRAIN (source) datasets")
        self.train = []
        self._num_train_pids = 0
        self._num_train_cloth_ids = 0
        ...
        for name in self.source_names:
            ...
            for img_path, pid, cloth_id, camid in dataset.train:
                pid += self._num_train_pids
                cloth_id += self._num_train_cloth_ids
                camid += self._num_train_cams
                self.train.append((img_path, pid, cloth_id, camid))

            self._num_train_pids += dataset.num_train_pids
            self._num_train_cloth_ids += dataset.num_train_cloth_ids
            self._num_train_cams += dataset.num_train_cams

        if self.train_sampler == 'RandomIdentitySampler':
            self.trainloader = DataLoader(
                ImageDataset(self.train, transform=transform_train, training=True),
        ...
        for name in self.target_names:
            ...
            self.testloader_dict[name]['query'] = DataLoader(
                ImageDataset(dataset.query, transform=transform_test, training=False),
            ...
            self.testloader_dict[name]['gallery'] = DataLoader(
                ImageDataset(dataset.gallery, transform=transform_test, training=False),
                batch_size=self.test_batch_size, shuffle=False, num_workers=self.workers,
                pin_memory=self.pin_memory, drop_last=False
            )
        ...
        print("  # train cloth_ids: {}".format(self._num_train_cloth_ids))
...
class VideoDataManager(BaseDataManager):
    ...
    def __init__(self,
                 ...
                 image_training=False # train the video-reid model with images rather than tracklets
                 ):
        ...
        # Build train and test transform functions
        transform_train = build_transforms(self.height, self.width, is_train=False) # Xin added
        ...
        if image_training:
            ...
            # each batch has image data of shape (batch=N*K, seq_len, channel, height, width)
            self.trainloader = DataLoader(
